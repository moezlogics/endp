...
import PrescriptionUploader from "@modules/checkout/components/prescription-uploader"
...
export function CheckoutFormClient({ cart, customer, shippingMethods, paymentMethods }: Props) {
  ...
  const hasPrescription = !!(cart.metadata as any)?.prescription_url
  const missingPrescription = requiresPrescription && !hasPrescription

  return (
    <form action={formAction} className="w-full pb-24 lg:pb-0">
      {/* ── Prescription upload (auto-renders only when needed) ── */}
      <PrescriptionUploader cart={cart} />
      ...
      {missingPrescription && (
        <div className="mb-4 flex items-center gap-2.5 p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs font-semibold">
          <i className="ph-fill ph-warning-circle text-base text-rose-600 shrink-0" aria-hidden />
      ...
      <div className="flex items-center justify-between mt-8 pt-4 border-t border-line">
        ...
        <button
          type="submit"
          disabled={pending || missingPrescription}
