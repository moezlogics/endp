...
/**
 * Scans the filesystem to discover all custom/static polio pages dynamically.
 */
function getPolioPages(): string[] {
  const walkUrls: string[] = []
  const mainDir = path.join(process.cwd(), "src", "app", "[countryCode]", "(main)")
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          if (
            content.includes("PolioInfoPage") ||
            pathLower.includes("polio") ||
            pathLower.includes("faq") ||
            pathLower.includes("certificate") ||
            pathLower.includes("component")
          ) {
            const relative = path.relative(mainDir, fullPath).replace(/\\/g, "/").replace(/\/page\.tsx$/, "")
          ...
          // Silent catch to prevent build breaking if files are read during compilation
        }
      }
    }
  }
...
async function buildIndexChunk(
  ...
  // Dynamically discover and add static polio info pages
  try {
    const staticSet = new Set(STATIC_PATHS.map((p) => (p === "" ? "/" : p)))
    const polioPages = getPolioPages()
    for (const pagePath of polioPages) {
      if (!staticSet.has(pagePath)) {
        entries.push(
          entry(
            baseUrl,
            pagePath,
    ...
    console.error("[sitemap] Failed to discover and register dynamic polio pages", e)
  }
