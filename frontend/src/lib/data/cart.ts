...
/**
 * Saves a prescription URL (from CDN) to cart metadata so the backend
 * order-placed subscriber can clone it. Validates that the cart is not
 * missing.
 *
 * Pass `null` for `url` to remove an existing prescription (customer
 * replaces the file).
 */
export async function setPrescriptionUrl(url: string | null) {
  const cartId = await getCartId()
  if (!cartId) throw new Error("No existing cart found")

  // Preserve existing metadata; only flip the prescription fields.
  const existing = await retrieveCart(cartId, "metadata")
  const merged: Record<string, any> = {
    ...((existing?.metadata as Record<string, any>) || {}),
    prescription_url: url || null,
    prescription_uploaded_at: url ? new Date().toISOString() : null,
  }

  await updateCart({ metadata: merged } as any)
}
...
export async function addBundleToCart({
  ...
  const backendUrl =
    process.env.MEDUSA_BACKEND_URL || "http://localhost:3052"
